...
class Seq2SeqTransformerDecoder(nn.Module):
    def __init__(self,
        ...
        super(Seq2SeqTransformerDecoder, self).__init__()
        self._vocab = vocab
    ...
    def forward(self, encoded_hidden_embeddings: torch.Tensor,  hidden: torch.Tensor, cell: torch.Tensor,
        ...
        # B, 1, Ed + He
        input_embeddings = self._input_mlp(input_label_embeddings)
        ...
        for step in range(decode_max_seq_len):
            ...
            # B, 1, Ed
            merge_label_embeddings = H * pred_label_embeddings + (1 - H) * soft_label_embeddings
            if step > 0:
                input_label_embeddings = torch.cat([input_label_embeddings, merge_label_embeddings], dim=1)
            else:
