...
class Seq2KVAttn2Decoder(nn.Module):
    def __init__(self,
                 vocab: Vocabulary,
        ...
        super(Seq2KVAttn2Decoder, self).__init__()
        self._vocab = vocab
    ...
    def forward(self, encoded_hidden_embeddings: torch.Tensor, hidden: torch.Tensor, cell: torch.Tensor,
        ...
        for step in range(decode_max_seq_len):
            ...
            input_label_embeddings = input_label_embeddings + attn_last_label_embeddings  # AD2
            # # B, 1, Ed + He
            input_embeddings = torch.cat((input_label_embeddings, input_encoder_hidden_embeddings), dim=-1)
        ...
        # sparce_key_similar = key_similars.masked_fill(key_similars_mask, 0)  # 将similar < 0 dont activation
        # # sparce_key_similar = sparce_key_similar.masked_fill(~key_similars_mask, 1)  # similar > 0 activation
