...
from entity_typing.modules import Seq2SeqDecoder, Seq2SeqAttnDecoder, Seq2SeqAttn2Decoder, Seq2SeqLoss, Seq2SeqTransformerDecoder, Seq2SeqTransformerDecoder2
from entity_typing.constant import NAME_SPACE_SEQ_LABEL, FEATURE_NAME_SEQ_LABEL, EOS_SYMBOL, PADDING_TOKEN

logger = logging.getLogger(__name__)


@Model.register("seq_elmo_typing_model")
class SeqElmoTypingModel(Model):
    def __init__(self,
                 vocab: Vocabulary,
                 context_field_embedder: TextFieldEmbedder,  # bert
                 mention_field_embedder: TextFieldEmbedder,
        ...
        if decoder_type == 'lstm':
            self._decoder = Seq2SeqAttnDecoder(
                ...
                input_size=self._label_field_embedder.get_output_dim() * 1 + self._context_field_embedder.get_output_dim(),
                decoder_hidden_size=self._context_field_embedder.get_output_dim() + self._mention_encoder.get_output_dim() + self._mention_char_encoder.get_output_dim(),
                encoder_hidden_dim=self._context_field_embedder.get_output_dim(),
        ...
        if decoder_type == 'lstm2':
            self._decoder = Seq2SeqAttn2Decoder(
                vocab=vocab,
                label_field_embedder=self._label_field_embedder,
                label_embedding_dim=self._label_field_embedder.get_output_dim(),
                input_size=self._label_field_embedder.get_output_dim() * 1 + self._context_field_embedder.get_output_dim(),
                decoder_hidden_size=self._context_field_embedder.get_output_dim() + self._mention_encoder.get_output_dim() + self._mention_char_encoder.get_output_dim(),
                # decoder_hidden_size=self._context_field_embedder.get_output_dim(),
                encoder_hidden_dim=self._context_field_embedder.get_output_dim(),
                output_size=vocab.get_vocab_size(NAME_SPACE_SEQ_LABEL),
                dropout_rate=decoder_dropout,
                teaching_forcing_rate=teaching_forcing_rate,
    ...
    def forward(self, context, mention, mention_char,
        ...
        # mention
        mention_token_mask = get_text_field_mask(mention)
        mention_char_mask = get_text_field_mask(mention_char)
        mention_hidden = self._mention_encoder(self._mention_field_embedder(mention), mention_token_mask)
        mention_char_hidden = self._mention_char_encoder(self._mention_char_field_embedder(mention_char), mention_char_mask)
        # (batch_size, token_dim + char_dim)
        mention_hidden = torch.cat([mention_hidden, mention_char_hidden], dim=-1)
        ...
        context_hidden = self._context_seq_encoder(context_embedding, context_mask)  # attention ELMo
        # context_hidden = context_embedding  # attention LSTM,

        init_represent, _ = self._context_vec_encoder(context_hidden, context_mask)
        # concat mention hidden
        # (batch_size, encoder_dim + token_dim + char_dim)
        init_represent = torch.cat([init_represent, mention_hidden], dim=-1)
