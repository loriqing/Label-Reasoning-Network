...
from entity_typing.modules import Seq2KVDecoder, Seq2KVDecoderUF, Seq2KVAttnDecoder, Seq2KVAttn2Decoder, Seq2SeqLoss, Seq2KVAttn2DecoderUF
...
@Model.register("seq_kv_typing_model_uf")
class SeqKVTypingModelUF(Model):
    def __init__(self,
                 vocab: Vocabulary,
        ...
        if decoder_type == 'lstm2':
            self._decoder = Seq2KVAttn2DecoderUF(
                vocab=vocab,
                label_field_embedder=self._label_field_embedder,
                label_embedding_dim=self._label_field_embedder.get_output_dim(),
                input_size=self._label_field_embedder.get_output_dim() + self._context_vec_encoder.get_output_dim(),
                decoder_hidden_size=self._context_vec_encoder.get_output_dim(),
                encoder_hidden_dim=self._context_seq_encoder.get_output_dim(),
                output_size=vocab.get_vocab_size(NAME_SPACE_SEQ_LABEL),
                dropout_rate=decoder_dropout,
