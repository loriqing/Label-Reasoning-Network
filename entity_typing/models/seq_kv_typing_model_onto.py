...
from entity_typing.modules import Seq2KVDecoder, Seq2KVDecoderOnto, Seq2KVAttnDecoder, Seq2KVAttn2Decoder, Seq2SeqLoss, Seq2KVAttn2DecoderOnto
...
@Model.register("seq_kv_typing_model_onto")
class SeqKVTypingModelOnto(Model):
    def __init__(self,
                 vocab: Vocabulary,
        ...
        if decoder_type == 'lstm2':
            self._decoder = Seq2KVAttn2DecoderOnto(
                vocab=vocab,
                label_field_embedder=self._label_field_embedder,
                label_embedding_dim=self._label_field_embedder.get_output_dim(),
        ...
        if decoder_type == 'lstm0':
            self._decoder = Seq2KVDecoderOnto(
                vocab=vocab,
                label_field_embedder=self._label_field_embedder,
                label_embedding_dim=self._label_field_embedder.get_output_dim(),
                input_size=self._label_field_embedder.get_output_dim() * 1 + self._context_seq_encoder.get_output_dim(),
