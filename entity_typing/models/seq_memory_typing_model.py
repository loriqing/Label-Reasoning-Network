...
from entity_typing.modules import Seq2MemAttnDecoder, Seq2MemAttn2Decoder, Seq2SeqLoss, Seq2SeqTransformerDecoder, Seq2SeqTransformerDecoder2
from entity_typing.constant import NAME_SPACE_SEQ_LABEL, FEATURE_NAME_SEQ_LABEL, EOS_SYMBOL, PADDING_TOKEN, \
    FEATURE_NAME_MEMORY, NAME_SPACE_MEMORY

logger = logging.getLogger(__name__)


@Model.register("seq_memory_typing_model")
class SeqMemoryTypingModel(Model):
    def __init__(self,
                 vocab: Vocabulary,
                 context_field_embedder: TextFieldEmbedder,  # bert
                 label_field_embedder: TextFieldEmbedder,
                 memory_field_embedder: TextFieldEmbedder,  # pretrained word embedding
        ...
        self._memory_field_embedder = memory_field_embedder
        ...
        if decoder_type == 'lstm':
            self._decoder = Seq2MemAttnDecoder(
                ...
                memory_dim=self._memory_field_embedder.get_output_dim(),
                label_namespace=[NAME_SPACE_SEQ_LABEL, FEATURE_NAME_SEQ_LABEL],
                detach=detach,
            )
        if decoder_type == 'lstm2':
            self._decoder = Seq2MemAttn2Decoder(
                vocab=vocab,
                label_field_embedder=self._label_field_embedder,
                label_embedding_dim=self._label_field_embedder.get_output_dim(),
                input_size=self._label_field_embedder.get_output_dim() * 1 + self._context_seq_encoder.get_output_dim(),
    ...
    def calculate_mem2seq_dic(self, vocab):
        ...
        memory_vocab = vocab._token_to_index[NAME_SPACE_MEMORY]
        mem2seq_dic = {}
    ...
    def forward(self, context,
                ...
                memory: Dict[str, Dict[str, torch.Tensor]] = None, memory_labels=None,
                labels=None, fine_labels=None, ultra_fine_labels=None) -> Dict:

        if memory_labels is not None:
            # (batch_size, memory_len)
            memory_mask = get_text_field_mask(memory)
            # (batch_size, memory_len, memory_emb)
            memory_embedding = self._memory_field_embedder(memory)
            # (batch_size, memory_len)
            memory_labels = memory_labels.masked_fill(~memory_mask, 0)
        else:
            memory_mask = None
        ...
        output, decode_ids, similars = self._decoder(encoded_hidden_embeddings=context_hidden,
        ...
        pred_scores, measure_scores = utils.convert_memory_output(output, decode_ids, seq_label_mask, self.similar_thred,
                                                                  similars, memory[FEATURE_NAME_MEMORY]['tokens'], self.mem2seq_dic,
        ...
        memory_loss = self.memory_loss(similars=similars, gold_labels=memory_labels, memory_mask=memory_mask)
        loss = loss + self._loss_lambda * memory_loss
        if loss is not None:
