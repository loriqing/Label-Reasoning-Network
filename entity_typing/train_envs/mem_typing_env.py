...
from entity_typing.dataReader import SeqTypingReader, \
    MarMemoryTypingReader, SepMemoryTypingReader, MarSoftMemoryTypingReader, SepSoftMemoryTypingReader
from entity_typing.models import SeqTypingModel, SeqBertTypingModel, SeqMemoryTypingModel, SeqKVTypingModel, SeqKVTypingModelOnto, SeqKVTypingModelUF
...
class MemTypingTrainEnv():
    ...
    def add_arguments(parser):
        ...
        group.add_argument("-loss-lambda", dest='loss_lambda', type=float, default=0, help="Memory loss weight")
        group.add_argument("-loss-lambda2", dest='loss_lambda2', type=float, default=1, help="Value loss weight")
        ...
        group.add_argument("-bert-thred", dest='bert_thred', type=float, default=0.1)
        group.add_argument("-sparce-rate", dest='sparce_rate', type=float, default=0.8)
        group.add_argument("-value-file", dest='value_file', type=str, default=None)
        group.add_argument('-show', dest='show', action='store_true')
    ...
    def prepare_model(args, vocab):
        ...
        if args.bert_type == 'none':
            ...
            if args.bert_type == ['entity_marker_memory', 'sep_mention_memory']:
                memory_field_embedder = module_utils.prepare_memory_embedder(args, vocab)
                model = SeqMemoryTypingModel(
                        vocab=vocab,
                        context_field_embedder=context_field_embedder,
                        label_field_embedder=label_field_embedder,
                        memory_field_embedder=memory_field_embedder,
                        ...
                        detach=args.detach,
                        similar_thred=args.similar_thred,
                )
            elif args.bert_type in ['entity_marker_kv', 'sep_mention_kv']:
                key_field_embedder = module_utils.prepare_key_embedder(args, vocab)
                value_field_embedder = module_utils.prepare_value_embedder(args, vocab)
                if "ontonotes" in args.data_folder_path:
                    model = SeqKVTypingModelOnto(
                        vocab=vocab,
                        context_field_embedder=context_field_embedder,
                        label_field_embedder=label_field_embedder,
                        key_field_embedder=key_field_embedder,
                        value_field_embedder=value_field_embedder,
                        context_vec_encoder=context_vec_encoder,
                        context_seq_encoder=context_seq_encoder,
                        dropout_rate=args.dropout,
                        label_namespace=label_namespace,
                        ...
                        evaluation=args.evaluation,
                        loss_lambda=args.loss_lambda,
                        loss_lambda2=args.loss_lambda2,
                        detach=args.detach,
                        similar_thred=args.similar_thred,
                        sparce_rate=args.sparce_rate,
                    )
                else:
                    model = SeqKVTypingModel(
                            vocab=vocab,
                            context_field_embedder=context_field_embedder,
                            label_field_embedder=label_field_embedder,
                            key_field_embedder=key_field_embedder,
    ...
    def prepare_dataset_reader(args) -> DatasetReader:
        ...
        if args.bert_type == 'entity_marker_memory':
            ...
            memory_indexers = data_utils.prepare_memory_indexer(args)
            reader = MarMemoryTypingReader(lazy=args.lazy,
                                           tokenizers=tokenizers,
                                           indexers=indexers,
                                           label_indexers=label_indexers,
                                           memory_indexers=memory_indexers,
                                           max_sentence_length=args.max_sentence_length,
                                           test_ins=args.test_ins,
                                           seq_type=args.seq_type,
                                           shuffle_num=args.shuffle_num,
                                           )
        elif args.bert_type == 'sep_mention_memory':
            tokenizers = data_utils.prepare_tokenizer(args, token=False, char=False, transformer=True)
            indexers = data_utils.prepare_token_indexers(args, token=False, pos_tag=False, char=False,
                                                         transformer=transformer, mismatch=False)
            memory_indexers = data_utils.prepare_memory_indexer(args)
            reader = SepMemoryTypingReader(lazy=args.lazy,
                                           tokenizers=tokenizers,
                                           indexers=indexers,
                                           label_indexers=label_indexers,
                                           memory_indexers=memory_indexers,
        ...
        elif args.bert_type == 'entity_marker_kv':
            tokenizers = data_utils.prepare_tokenizer(args, token=False, char=False, transformer=True)
            indexers = data_utils.prepare_token_indexers(args, token=False, pos_tag=False, char=False,
                                                         transformer=transformer, mismatch=False)
            memory_indexers = data_utils.prepare_kv_indexer(args)
            reader = MarSoftMemoryTypingReader(lazy=args.lazy,
                                               tokenizers=tokenizers,
                                               indexers=indexers,
                                               label_indexers=label_indexers,
                                               memory_indexers=memory_indexers,
                                               ...
                                               bert_thred=args.bert_thred,
                                               )
        elif args.bert_type == 'sep_mention_kv':
            tokenizers = data_utils.prepare_tokenizer(args, token=False, char=False, transformer=True)
            indexers = data_utils.prepare_token_indexers(args, token=False, pos_tag=False, char=False,
                                                         transformer=transformer, mismatch=False)
            memory_indexers = data_utils.prepare_kv_indexer(args)
            reader = SepSoftMemoryTypingReader(lazy=args.lazy,
                                               tokenizers=tokenizers,
                                               indexers=indexers,
                                               label_indexers=label_indexers,
                                               memory_indexers=memory_indexers,
            ...
            raise ValueError('no such bert_type!')
        return reader

    @staticmethod
    def train_model(args):
        reader = MemTypingTrainEnv.prepare_dataset_reader(args)
        ...
        vocab, mapping_dic = data_utils.modify_vocab(vocab=vocab, from_namespace=label_namespace,
                                                     ...
                                                     add_tokens=[MEMORY_SYMBOL, UNK_SYMBOL, EOS_SYMBOL])
        ...
        train_loader, valid_loader, test_loader = data_utils.build_dataloader(args, train_dataset, valid_dataset,
                                                                              test_dataset)
        model = MemTypingTrainEnv.prepare_model(args, vocab)
